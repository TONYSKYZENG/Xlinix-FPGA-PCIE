...
u32 XExample_Get_ver(XExample *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XExample_ReadReg(InstancePtr->Bus_a_BaseAddress, XEXAMPLE_BUS_A_ADDR_VER_DATA);
    return Data;
}

u32 XExample_Get_ver_vld(XExample *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XExample_ReadReg(InstancePtr->Bus_a_BaseAddress, XEXAMPLE_BUS_A_ADDR_VER_CTRL);
    return Data & 0x1;
}
